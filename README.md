...
$k + hash\_table\_size \cdot \alpha = r \cdot k$, где $\alpha$ - какое-то целое число. Наименьшее $k$, когда это выполняется - $r \cdot k = $LCM(k, hash\_table\_size)$, то есть $r \cdot k = \frac{k \cdot hash\_table\_size}{GCD(k, hash\_table\_size)}$ или $r = \frac{hash\_table\_size}{GCD(k, hash\_table\_size)}$. 
